...
def vecDet(vector_raw):
  det = 0
  for one_hot in vec.values():
      det += one_hot ** 2
  else:
...
def cos_sim(v0,v1):
  ...
  cos = numerator / (vecDet(v0) * vecDet(v1))
...
def create_ppmi_vectors(wids, o_counts, co_counts, tot_count):
    ...
    for wid0 in wids:
        ...
        for coword, conum in co_counts[wid0].items():
            pmi_value = PMI(conum, o_counts[wid0], o_counts[coword], tot_count)
            if pmi_value > 0:
                vector_dict[wid0][coword] = pmi_value
    return vector_dict
